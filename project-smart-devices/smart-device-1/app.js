...
sensorService.init().then(() => {
    subscriptionService.init().then(() => {
        node_cron_1.default.schedule(Environment_1.Env.SENSOR_CRON, () => {
            sensorService.activate().then((measurement) => {
                console.log(measurement);
                subscriptionService.forEach((subscription) => {
                    axios_1.default.post(subscription.consumerEndpoint, measurement).catch((err) => console.error(err));
                });
            });
        });
    });
...
server.listen(Environment_1.Env.SMARTDEV1_PORT, () => console.log(`Smart Device 1 is listening on port ${Environment_1.Env.SMARTDEV1_PORT}`));
